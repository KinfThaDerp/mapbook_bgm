from mapbook_lib import controller as mcon
from tkinter import *
...
button_pokaz_szczegoly = Button(ramka_lista_obiektow, text="Pokaz Szczegoly", command=lambda: mcon.user_details(users, list_box,  label_imie_szczegoly_obiektu_wartosc, label_lokalizacja_szczegoly_obiektu_wartosc, label_posty_szczegoly_obiektu_wartosc, map_widget))
button_pokaz_szczegoly.grid(row=2,column=0)

button_usun_obiekt = Button(ramka_lista_obiektow, text="Usuń Obiekt", command=lambda: mcon.delete_user(users, list_box))
button_usun_obiekt.grid(row=2,column=1)

button_edytuj_obiekt = Button(ramka_lista_obiektow, text="Edytuj Obiekt", command=lambda: mcon.edit_user(users, entry_name, entry_location, entry_posty, entry_img_url, button_dodaj_obiekt, list_box))
...
button_dodaj_obiekt = Button(ramka_formularz, text="Dodaj obiekt", command=lambda: mcon.add_user(users, entry_name, entry_location, entry_posty, entry_img_url, map_widget, list_box))
